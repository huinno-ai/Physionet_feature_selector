...
from team_code import load_twelve_lead_model, load_six_lead_model, load_three_lead_model, load_two_lead_model, eval_dots
...
def test_model(model_directory, data_directory, output_directory):
    ...
    for i in range(num_recordings):
        ...
        if all(lead in leads for lead in twelve_leads):
            args = run_twelve_lead_model(twelve_lead_model, header, recording, header_files[i])
            if args is None:
                continue
            classes, labels, probabilities = args
...
def test_eval_dots(model_directory, data_directory, output_directory):
    eval_dots()
...
if __name__ == '__main__':
    ...
    test_eval_dots(model_directory,data_directory,output_directory)
